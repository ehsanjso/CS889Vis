...
export default class TimeSpiral {
  ...
  _init() {
    ...
    this._layerHeight =
      (max - this._innerRadius) / (this._layers + 1) - this._charBox.height;
  ...
  _initColors() {
    ...
    if (style.colorBy === "time") {
      const months = d3.timeMonth.every(1).range(this._firstDay, this._lastDay);
      ...
      this._color = d3
        ...
        .domain(
          style.reverseColor ? [this._max, this._min] : [this._min, this._max]
        );
    }
  }
  ...
  _renderBars() {
    ...
    var color;
    if (this._style.colorBy === "value") color = (d) => this._color(d.value);
    else
      color = (d) => {
